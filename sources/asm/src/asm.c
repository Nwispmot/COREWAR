...
void	parse_args2(t_parser *par, int i)
{
	...
	par->file[par->y][i] == '\t'))
		i++;
	if (par->file[par->y][i] != '\0' && par->file[par->y][i] != '#')
...
void	parse_args(t_parser *par, int i, int args, int x)
{
	char	*string;

	while (par->file[par->y][x] != '\0' && par->file[par->y][x] != COMMENT_CHAR)
		x++;
	while (par->file[par->y][par->x] != '\0' && par->x < x)
	{
		if (args >= g_op[par->oper].args_num)
			break ;
		while (par->file[par->y][i] != SEPARATOR_CHAR && par->file[par->y][i]
		!= ' ' && par->file[par->y][i] != '\t' && par->file[par->y][i] &&
		par->file[par->y][i] != COMMENT_CHAR)
			i++;
